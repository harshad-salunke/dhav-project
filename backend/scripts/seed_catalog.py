"""
Seed 50 kirana catalog items into Supabase PostgreSQL.
...
async def seed():
    ...
    print(f"Seeding {len(CATALOG_ITEMS)} catalog items into Supabase PostgreSQL...\n")
    inserted = 0
    skipped = 0
        ...
        try:
            await conn.execute("""
                INSERT INTO catalog_items
                    (id, name, name_hindi, name_marathi, category, unit, price, image_url, is_active, created_at)
                VALUES ($1,$2,$3,$4,$5,$6,$7,'',true,0)
            """,
                item_id,
                item["name"], item["name_hindi"], item["name_marathi"],
                item["category"], item["unit"], float(item["price"]),
            )
