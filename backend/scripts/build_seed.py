#!/usr/bin/env python3
"""
DHAV seed generator → backend/migrations/005_seed_marketplace.sql
...
OUT = os.path.join(os.path.dirname(__file__), "..", "migrations", "005_seed_marketplace.sql")
...
def main():
            ...
            for si, (sub_name, products) in enumerate(subs.items()):
                ...
                for pi, (base_name, brand_spec, unit, lo, hi) in enumerate(products):
                    ...
                    for bi, brand in enumerate(brand_list):
                        ...
                        for vi, (vunit, mult) in enumerate(pack_variants(market, unit, len(brand_list))):
    ...
    lines = []
    lines.append("-- DHAV: Multi-marketplace seed (categories, subcategories, products, demo stores)")
    ...
    lines.append(f"-- Totals: {len(cat_rows)} categories | {len(sub_rows)} subcategories | "
                 ...
                 f"{len(customer_rows)} demo customers")
    for m in MARKETS:
        lines.append(f"--   {m:12s} {cat_market_cnt[m]:3d} categories | {by_market_cnt[m]:4d} products")
    lines.append("--")
    lines.append("-- ╔══════════════════════════════════════════════════════════════════════════╗")
    lines.append("-- ║  SAFE TO RE-RUN: this block first REMOVES the previous seed (only rows     ║")
    lines.append("-- ║  whose id starts with cat_/sub_/itm_/store_ — your admin-created products  ║")
    lines.append("-- ║  use random UUID ids and are NOT touched), then re-inserts everything.     ║")
    lines.append("-- ║  Just run the WHOLE file once — no need to hunt for 'new' rows.            ║")
    lines.append("-- ╚══════════════════════════════════════════════════════════════════════════╝")
    lines.append("DELETE FROM catalog_items WHERE starts_with(id, 'itm_');")
    lines.append("DELETE FROM subcategories WHERE starts_with(id, 'sub_');")
    lines.append("DELETE FROM categories    WHERE starts_with(id, 'cat_');")
    lines.append("DELETE FROM stores        WHERE starts_with(id, 'store_');")
    lines.append("DELETE FROM users         WHERE starts_with(uid, 'cust_demo_');")
    lines.append("")
    ...
    def _item_val(it):
        return (f"({sql_str(it['id'])},{sql_str(it['name'])},'','',{sql_str(it['category_name'])},"
                ...
                f"{sql_str(json.dumps(it['specs']))}::jsonb,true,{NOW},{NOW})")
    ...
    _ITEM_INSERT = ("INSERT INTO catalog_items (id, name, name_hindi, name_marathi, category, marketplace_type, "
                    ...
                    "stock_quantity, image_url, images, specs, is_active, created_at, updated_at) VALUES")
    ...
    # ── 1) combined file (best for psql / CLI; too big for Supabase SQL editor) ──
    combined = (lines + cat_block() + sub_block() + items_block(item_rows, banner=True)
                + store_block() + customers_block())
    with open(os.path.abspath(OUT), "w", encoding="utf-8") as f:
        f.write("\n".join(combined))

    # ── 2) split files for the Supabase SQL editor (each well under its ~1 MB cap) ──
    parts_dir = os.path.join(os.path.dirname(os.path.abspath(OUT)), "seed_parts")
    os.makedirs(parts_dir, exist_ok=True)

    def write_part(fname, body_lines):
        with open(os.path.join(parts_dir, fname), "w", encoding="utf-8") as f:
            f.write("\n".join(body_lines))

    # part 1: reset + categories + subcategories
    write_part("005_part1_taxonomy.sql",
               part_header("Part 1/7 — reset + categories + subcategories",
                           "Run FIRST. Wipes old seed rows (cat_/sub_/itm_/store_) then inserts the taxonomy.")
               + RESET + cat_block() + sub_block())

    # parts 2-5: one product file per marketplace
    market_files = {"grocery": "005_part2_products_grocery.sql",
                    "fruits": "005_part3_products_fruits.sql",
                    "electronics": "005_part4_products_electronics.sql",
                    "pharmacy": "005_part5_products_pharmacy.sql"}
    for i, (m, fname) in enumerate(market_files.items(), start=2):
        sub_items = [it for it in item_rows if it["market"] == m]
        write_part(fname,
                   part_header(f"Part {i}/7 — {m} products ({len(sub_items)})",
                               "Run after Part 1.") + items_block(sub_items, banner=False))

    # part 6: demo stores (run after products — their available_item_ids reference them)
    write_part("005_part6_stores.sql",
               part_header("Part 6/7 — demo stores", "Run after the product parts.")
               + store_block())

    # part 7: demo customers (for the admin Coverage map — clusters + coverage)
    write_part("005_part7_customers.sql",
               part_header("Part 7/7 — demo customers (Coverage map)",
